...
### 	COPY THIS to your C:/Users/JohnDoe (or equivalent), one level above Auto-GPT folder, for third-party imports to work
...
#
# N/A*: CUDA OOM, >>24 GB VRAM   D = default -> Got 12 GB VRAM? You probably won't need to adjust anything below.
# I recommend a test run from CMD before use in Auto-GPT, like so: python CLIP.py --image_path "C:/Users/JohnDoe/your_test_image.png"

clipmodel = 'RN50x4'
# available models = ['RN50', 'RN101', 'RN50x4', 'RN50x16', 'RN50x64', 'ViT-B/32', 'ViT-B/16', 'ViT-L/14', 'ViT-L/14@336px'] -- if not, upgrade from git:OpenAI/CLIP

training_iterations = 200
#  20: nysdits fry tamedecihpc vector inhibitxl ctr gawd gear maniport -- in other words, don't do that.
#  50: Activations still kinda all over the place. Can be fun. Weird gibberish tokens still ensued.
# 200: Default. A solid CLIP opinion. Good for e.g. GPT prompting text-to-image AI. For visual image websearch, consider more:
# 400: Use for complex images, especially if they also have embedded text, e.g. store fronts. CLIP *can* read, but needs to take "a good look".
# 600: Pretty stable loss, but at this point takes minutes rather than seconds to train CLIP on the image.

batchsize = 16
# min = 4, default = 16, max = if you got that much VRAM, consider using RN50x4 with 32 -- in my observation (subjective) the best "value for VRAM" in high-end GPUs / RTX 3090 + RTX 4090
...
from special.ldm.util import parallel_data_prefetch
...
displ(F.interpolate(im, (sideX, sideY))[0].cpu().detach().numpy())
...
def checkin(loss, lll):
    ...
    with open(f"tokens_{img_name}.txt", "w", encoding='utf-8') as f:
